...
def main():
    parser = argparse.ArgumentParser(
        description="Helper script to run the datbase and web frontend in separate threads."
    )

    # Add --config runtime argument
    parser.add_argument(
        "--config", help="Path to the configuration file.", default="config.ini"
    )
    args = parser.parse_args()

    # Database Thread
    dbthrd = threading.Thread(
        target=run_script, args=("startdb.py", "--config", args.config)
    )

    # Web server thread
    webthrd = threading.Thread(
        target=run_script, args=("main.py", "--config", args.config)
    )
